...
int bwa_pemerge(const pem_opt_t *opt, bseq1_t x[2])
{
	...
	for (i = 0; i < x[0].l_seq; ++i) {
		int c = x[0].seq[i];
		s[0][i] = c < 0 || c > 127? 4 : c <= VALUE_DOMAIN ? c : aa_encode_hash[c];
		q[0][i] = x[0].qual? x[0].qual[i] - 33 : opt->q_def;
	}
	for (i = 0; i < x[1].l_seq; ++i) {
		int c = x[1].seq[x[1].l_seq - 1 - i];
		c = c < 0 || c > 127? 4 : c < VALUE_DEFINED ? c : aa_encode_hash[c];
		s[1][i] = c < VALUE_DEFINED? VALUE_DEFINED - 1 - c : VALUE_DEFINED;
